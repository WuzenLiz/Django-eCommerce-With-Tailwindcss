...
class userAddressBook(models.Model):
 ...
 def save(self, *args, **kwargs):
  if userAddressBook.objects.filter(user=self.user).all().count() == 0 and self.is_main_address == False:
   ...
   userAddressBook.objects.filter(user=self.user, is_main_address=True).update(is_main_address=False)
...
class userInfomation(models.Model):
 ...
 def save(self, *args, **kwargs):
  if self.phone == None:
   self.phone = self.user.useraddressbook.objects.filter(user=self.user, is_main_address=True).first().phone
  else:
    pass
